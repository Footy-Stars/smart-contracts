...
contract MatchMaking {
    ...
    function quickMatch() external payable {
        ...
        matchDetail[matchId] = MatchDetail(
            msg.sender,
            address(0),
            msg.value,
            STATUS.NONE,
    ...
    function finishMatch(
        ...
        wagerResult[_id][m.matchWith] = WagerResult(
            m.player1,
            amount1,
            m.player2,
            amount2
        );
        wagerResult[m.matchWith][_id] = WagerResult(
            m.player1,
            amount1,
            m.player2,
            amount2
        ); // Repeat for be compatible both way
    ...
    function getMatchDetail(
        uint256 _id
    ) external view returns (MatchDetail memory) {
